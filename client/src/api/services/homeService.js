...
export const getHomes = async () => {
  try {
    ...
    logger.error(
      `Error fetching homes: ${error.response?.data?.error || error.message}`
    );
    throw new Error(error.response?.data?.error || "Failed to fetch homes");
  }
};

export const createHomeWithEnergyCertificate = async (
  homeData,
  ...
  try {
    ...
    logger.info(
      `Home and its energy certificate created successfully with ID: ${response.data.id}`
    );
    return response.data;
  } catch (error) {
...
export const updateHome = async (homeId, homeData) => {
  try {
    const response = await API.put(`/home/${homeId}`, homeData);
    logger.info(`Home updated successfully with ID: ${homeId}`);
    return response.data;
  } catch (error) {
    logger.error(
      `Error updating home with ID ${homeId}: ${
...
export const deleteHome = async (homeId) => {
  try {
    ...
    logger.error(
      `Error deleting home with ID ${homeId}: ${
        error.response?.data?.error || error.message
      }`
    );
